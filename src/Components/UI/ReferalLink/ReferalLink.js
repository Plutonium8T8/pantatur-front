...
//import { shareOnMobile } from "react-mobile-share";
import './ReferalLink.modules.css';
import { Icon } from '../';
...
const ReferalLink = () => {
    ...
    return (
        <div className='referal-link-container'>
            <div className='referal-text'>Реферальная ссылка</div>
            <div className='referal-more-text'>Отправьте эту ссылку своему другу чтобы получить кэшбэк за его активность</div>
            <div className='container-share-copy-tg'>
                <div className='button-share-tg'>
                    <button
                        className='btn-share-tg'
                        onClick={() =>
                            shareOnMobile({
                                text: "Страхуйся вместе со мной и получай кэшбэк",
                                url: `https://t.me/insurance_ru_bot?start=ref${tg_user_id}`,
                                title: "Блок страхования",
                                //images: [imgBase64],
                            })
                        }
                    >
                        <div className='container-share-text-tg'> <Icon name='share2' />  Поделиться </div>
                    </button>
                </div>
                <div className='button-copy-tg'>
                    <CopyToClipboard
                        text={`https://t.me/insurance_ru_bot?start=ref${tg_user_id}`}
                        onCopy={() => {
                            setShowPopover(true);
                        }}
                    >
                        <div>
                            {showPopover && <Popover message="Реферальная ссылка скопирована в буфер обмена" />}
